...
// Handle admin action on return requests
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_return_status'])) {
    $order_id = $_POST['order_id'];
    $return_status = $_POST['return_status'];

    // Update return_status based on admin decision
    $update_query = "UPDATE orders SET return_status = ? WHERE id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("si", $return_status, $order_id);
    $stmt->execute();

    // Redirect to prevent resubmission
    header("Location: show_user_orders.php");
    exit;
}
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_reply'])) {
    ...
    $update_reply_query = "UPDATE orders SET admin_reply = ? WHERE id = ?";
    $stmt = $con->prepare($update_reply_query);
    $stmt->bind_param("si", $admin_reply, $order_id);
    $stmt->execute();
...
<div class="container my-5 mx-4">
    ...
    <?php if ($row['return_status'] == 'Requested') { ?>
        <form method="POST">
            ...
            <button type="submit" name="update_return_status" class="btn btn-primary btn-sm mt-2">Update</button>
        </form>
    <?php } else { ?>
        <span class="<?= $row['return_status'] == 'Approved' ? 'text-success' : 'text-danger'; ?>">
            <?= htmlspecialchars($row['return_status']); ?>
        </span>
    <?php } ?>
</td>
<td>
    <?php if ($row['feedback']) { ?>
        <p><strong>User Feedback:</strong> <span class="text-success"><?= htmlspecialchars($row['feedback']); ?></span></p>
        <form method="POST">
            <input type="hidden" name="order_id" value="<?= $row['id']; ?>">
            <textarea name="admin_reply" class="form-control" rows="2" placeholder="Write your reply here"><?= htmlspecialchars($row['admin_reply']); ?></textarea>
            <button type="submit" name="submit_reply" class="btn btn-primary btn-sm mt-2">Reply</button>
        </form>
    <?php } else { ?>
